...
<div class="container p-4 bg-white rounded shadow-sm">
    ...
    <hr>
    @can('admin')
        <a href="{{ route('datek.create') }}" class="btn btn-primary mb-3">Input</a>
        <a href="{{ route('exportdatek') }}" class="btn btn-success mb-3">Print</a>

        {{-- <form action="{{ route('importdatek') }}" method="post" enctype="multipart/form-data">
    ...
    <table class="table table-bordered" id="datekTable">
...
@section('custom_script')
    <script>
        $(document).ready(function() {
            $('#datekTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('datek.getData') }}",
                columns: [
                    {data: 'id', name: 'id'},
                    {data: 'sn', name: 'sn'},
                    {data: 'ap', name: 'ap'},
